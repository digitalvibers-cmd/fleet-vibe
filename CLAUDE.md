...
- `server/src/Http/Controllers/Api/v1/OrderController.php` (novo) — tri metode:
  ...
  - `scanAssign(Request)` → `POST /v1/orders/scan-assign` `{ code }` — dodeljuje + dispečuje. **Važno (Fleetbase auth gotcha):** na `/v1/` token rutama `$request->user()` je `null` — firma i user se čitaju iz **sesije** koju postavlja `fleetbase.api` middleware: `session('company')` i `session('user')` (isti obrazac kao 22× `session('company')` u OrderController-u). Trenutni vozač = `Driver::where('user_uuid', session('user'))->where('company_uuid', session('company'))`. `DB::transaction` + `lockForUpdate()` (race-safe protiv dvostrukog claim-a): 409 ako je dodeljen DRUGOM vozaču (idempotentno ako je isti), inače `$order->assignDriver($driver, true)` (silent) pa `$order->firstDispatchWithActivity()`. **Dispatch je obavezan** — Orders lista u app-u filtrira `created` status, pa puko `driver_assigned_uuid` ne bi prikazalo porudžbinu.
