...
### LogiVibe core fleetops modifikacije (Customer-portal self-service bulk print otpremnica)

Customer sam iz korisničkog portala (`customer-portal/`) čekira svoje porudžbine na dashboard-u i štampa **isti** bulk PDF (`fleetops::labels/bulk`, isti QR) kao console Bulk Print — bez operatera kao posrednika. **Ne sme** da koristi console-ov `bulk-label` endpoint: taj je `withoutGlobalScopes` bez provere vlasništva, a customer token je pun company-scoped Sanctum token → direktan poziv bi curio TUĐE otpremnice po id-u. Zato **novi, ownership-scoped** endpoint. Embedded fleetops — pri upstream sync-u re-aplicirati ručno (`git log -- packages/fleetops/`):

- `server/src/Http/Controllers/Internal/v1/OrderController.php` — nova `customerBulkLabel(Request $request)`, kopija `bulkLabel()` (isti custom-field batch load, isti `fleetops::labels/bulk` view + DomPDF, isti `format` switch), **osim rezolucije porudžbina**: umesto slepog `whereIn(...)->withoutGlobalScopes()` dodaje CustomerOrders-direktivni predikat — `where('customer_uuid', session('user'))->orWhereHas('authenticatableCustomer', fn($q)=>$q->where('user_uuid', session('user')))`. Portal porudžbine imaju `customer_uuid = Contact.uuid`, pa je relevantna `orWhereHas` grana (isti obrazac kao `Auth/Directives/CustomerOrders.php`; direktive se NE primenjuju automatski na ručno pisane metode, zato eksplicitno). Tuđi id-evi tiho ispadaju, ne 403.
- `server/src/routes.php` — `$router->post('customer-bulk-label', $controller('customerBulkLabel'));` u `orders` grupi (pored `bulk-label`). Portal token prolazi istu `fleetbase.protected` grupu kao postojeći `/int/v1/orders` pozivi.

Frontend (van `packages/`, u `customer-portal/`, traje kroz upstream sync):
- `app/api/orders/labels/route.ts` (novi) — Next proxy route; `requireValidSession()`, **sirov** `fetch` na `/int/v1/orders/customer-bulk-label` sa Bearer + `{ ids, format:'stream' }`, vraća PDF `arrayBuffer` kao `application/pdf`. NE koristi `fleetbaseApi()` (radi `res.json()` → pokvario bi binarni PDF).
- `app/dashboard/page.tsx` — `selectedIds: Set<public_id>` state, checkbox po redu (karta prepakovana iz `<button>` u `<div>` sa checkbox + inner nav `<button>`), „Označi sve" kontrola, `handlePrintLabels()` (POST → blob → `window.open` nova kartica za štampu, fallback `<a download>`). Selekcija se briše na svaki `fetchOrders()` (da ne štampa sakrivene redove).
- `components/Header.tsx` — opcioni props `selectedCount`/`onPrintLabels`/`printing`; „Štampaj nalepnice (N)" dugme (`Printer` ikonica) renderovano SAMO kad je `onPrintLabels` prosleđen (tj. samo dashboard), **pre** „Uvezi Excel". Ostale stranice zovu `<Header/>` bez props → nema dugmeta.
